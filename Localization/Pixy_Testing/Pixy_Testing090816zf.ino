...
while(Calibrated<3){               //While the IMU isn't calibrated, wait for it to be calibrated (This is where we would need an automated routine)
    Calibrated = Cal_Status();
    Serial.print("Calibrating:  ");
    Serial.println(Calibrated);
    for (tomPos = 0; tomPos <= 45 && Calibrated < 3; tomPos += 1) { // goes from 0 degrees to 90 degrees
    // in steps of 1 degree
    tom.write(tomPos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
  for (jerryPos = 0; jerryPos <= 30 && Calibrated < 3; jerryPos += 1) { // goes from 0 degrees to 90 degrees
    // in steps of 1 degree
    jerry.write(jerryPos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
  for (tomPos = 30;  tomPos>= 0 && Calibrated < 3;  tomPos -= 1) { // goes from 90 degrees to 0 degrees
    tom.write(tomPos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
  for (jerryPos = 30;  jerryPos>= 0 && Calibrated < 3; jerryPos -= 1) { // goes from 90 degrees to 0 degrees
    jerry.write(jerryPos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
    delay(100);
